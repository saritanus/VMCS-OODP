			...
			if(item!=null){
				...
				if (current.getWeight() == c.getWeight()){
                                System.out.println("Iterator Pattern:When the coin has been entered and its weight has been checked then"
                                        + "traversal algorithm of findStoreIndex will be called that has been implemented in CashStoreIterator(ConcreteIterator) to find the index where the entered coin needs to be stored");
